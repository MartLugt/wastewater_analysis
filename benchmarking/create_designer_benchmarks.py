...
def main():
    ...
    parser.add_argument('--voc_perc', dest='voc_perc', required=True, type=float, help="VOC frequency (%) to be simulated")
    ...
    exclude_list = [name.split('.')[0] for name in VOC_names]

    full_df = read_metadata(args.metadata)
    selection_df = select_benchmark_genomes(full_df, exclude_list)
    ...
    if args.spike_only:
        ...
        for filename in VOC_files:
            VOC_name = filename.split('.')[0].split('/')[-1]
            print(filename + "  + " + VOC_name)
    ...
    for err_freq in err_frequencies:
        ...
        subprocess.check_call("art_illumina -ss HS25 -rs 0 -i {0} -l 150 -f {1} -p -o {2}/background_er{8}_ -m 250 -s 10 -qs {3} -qs2 {3} -ir {4} -ir2 {5} -dr {6} -dr2 {7}"
        ...
        for filename in VOC_files:
            ...
            print("Error rate: {} sub, {} ins, {} del ".format(sub_err, ins_err, del_err))
            subprocess.check_call("art_illumina -ss HS25 -rs 0 -i {0} -l 150 -f {1} -p -o {2}/{3}_er{9}_ -m 250 -s 10 -qs {4} -qs2 {4} -ir {5} -ir2 {6} -dr {7} -dr2 {8}"
                .format(voc_fasta, VOC_cov, args.outdir, VOC_name, quality_shift, insRate1, insRate2, delRate1, delRate2, err_freq), shell=True)
            print("\nMerging fastqs...")
            subprocess.check_call("cat {0}/background_er{2}_1.fq {0}/{1}_er{2}_1.fq > {0}/tmp1.fq"
                .format(args.outdir, VOC_name, err_freq, background_cov, VOC_cov), shell=True)
            subprocess.check_call("cat {0}/background_er{2}_2.fq {0}/{1}_er{2}_2.fq > {0}/tmp2.fq"
                .format(args.outdir, VOC_name, err_freq, background_cov, VOC_cov), shell=True)
            print("Shuffling reads...")
            subprocess.check_call("shuffle.sh in={0}/tmp1.fq in2={0}/tmp2.fq out={0}/wwsim_{1}_er{2}_1.fastq out2={0}/wwsim_{1}_er{2}_2.fastq overwrite=t fastawrap=0 ignorebadquality"
...
def select_benchmark_genomes(df, exclude_list):
    ...
    selection_df = df.loc[~df["Pango lineage"].isin(exclude_list)]
    print("\nLineage counts:")
